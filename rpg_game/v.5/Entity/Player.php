...
final class Player implements \Manager\DbManagerInterface
{
    ...
    /** @var \Model\Character */
    private ?\Model\Character $character = null;
    ...
    public static function get (int $iId) : ?object
    {
        ...
        $oStmt = $oPdo->prepare('SELECT id, name, score, character_info FROM `'. self::TABLE .'` WHERE id = :id');
        ...
        $oPlayer->setScore($aData['score']);
        $oPlayer->setCharacter(unserialize($aData['character_info']));
    ...
    public static function getByName (string $sName) : ?object
    {
        ...
        $oStmt = $oPdo->prepare('SELECT id, name, score, character_info FROM `'. self::TABLE .'` WHERE name = :name');
    ...
    public function save () : void 
    {
        ...
        if ($this->getId()) {
            // >> Si oui : UPDATE (> score)
            $oStmt = $oPdo->prepare('UPDATE `'. self::TABLE .'` SET score = :score, character_info = :character_info WHERE id = :id');
            $oStmt->bindValue(':id', $this->id, \PDO::PARAM_INT);
            $oStmt->bindValue(':score', $this->score, \PDO::PARAM_INT);
            $oStmt->bindValue(':character_info', serialize($this->character), \PDO::PARAM_STR);
            $oStmt->execute();
        } else {
            // >> Si non : INSERT INTO (> name, score)
            $oStmt = $oPdo->prepare('INSERT INTO `'. self::TABLE .'` ( name, score, character_info ) VALUES (:name, :score, :character_info)');
